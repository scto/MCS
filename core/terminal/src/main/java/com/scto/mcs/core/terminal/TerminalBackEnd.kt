...
import com.scto.mcs.feature.terminal.ui.terminalView
import com.scto.mcs.feature.terminal.ui.virtualKeysView
...
/**
 * Implementiert die Brücke zwischen dem Terminal-Emulator und der MCS UI.
 */
class TerminalBackEnd : TerminalViewClient, TerminalSessionClient {
    ...
    override fun onPasteTextFromClipboard(session: TerminalSession?) {
        ...
        if (clip.isNotBlank() && terminalView.get()?.mEmulator != null) {
    ...
    override fun onKeyDown(keyCode: Int, e: KeyEvent, session: TerminalSession): Boolean {
        if (keyCode == KeyEvent.KEYCODE_ENTER && !session.isRunning) {
            ...
            // Session-Handling Logik hier integrieren
            return true
        }
        return false
    }

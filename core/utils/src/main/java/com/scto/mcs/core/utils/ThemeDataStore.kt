...
import androidx.compose.ui.graphics.toArgb // Diese Erweiterungsfunktion muss eingeführt werden.
...
private val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "mcs_theme_settings")
...
class ThemeDataStoreRepository(private val context: Context) {
    ...
    val themeStateFlow: Flow<ThemeState> = context.dataStore.data
        .map { preferences ->
            ...
            // ✅ [Fixpunkt 1] Leselogik: Zuerst in Int konvertieren, dann den Color(Int)-Konstruktor verwenden. 
            // val name = valuel customColorValue = preferences[PreferencesKeys.CUSTOM_COLOR] ?: 0xFF6750A4
            val decodedColor = Color(customColorValue.toInt())

            LogCatcher.d("ThemeDebug_Repo", "Repo-Lesung: Monet=$isMonet, Custom=$isCustom, Style=$style, DecodedColorArg=${decodedColor.toArgb()}")
    ...
    suspend fun saveThemeConfig(
        ...
        // ✅ [Fixpunkt 2] Logik schreiben: Verwenden Sie .toArgb(), um einen Standard-Integer-Wert zu erhalten.
        // Dadurch wird der Unterschied verschleiert, ob die Farbe intern vom Typ Float oder Integer ist.
        val colorInt = customColor.toArgb()

        LogCatcher.w("ThemeDebug_Repo", "Repo-Schreiben: Monet=$isMonetEnabled, Custom=$isCustom, Style=$style, ColorInt=$colorInt")

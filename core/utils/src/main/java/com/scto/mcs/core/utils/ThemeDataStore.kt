...
class ThemeDataStoreRepository(private val context: Context) {
    ...
    val themeStateFlow: Flow<ThemeState> = context.dataStore.data
        .map { preferences ->
            ...
            // ✅ [Fix 1] Lese-Logik: Zuerst in Int umwandeln, dann den Color(Int)-Konstruktor verwenden
            val customColorValue = preferences[PreferencesKeys.CUSTOM_COLOR] ?: 0xFF6750A4
            val decodedColor = Color(customColorValue.toInt())

            Log.d("ThemeDebug_Repo", "Repo gelesen: Monet=$isMonet, Custom=$isCustom, Style=$style, DecodedColorArg=${decodedColor.toArgb()}")
    ...
    suspend fun saveThemeConfig(
        ...
        // ✅ [Fix 2] Schreib-Logik: .toArgb() verwenden, um einen Standard-Int-Wert zu erhalten
        // Dadurch wird der Unterschied verborgen, ob Color intern als Float oder Int repräsentiert wird
        val colorInt = customColor.toArgb()

        Log.w("ThemeDebug_Repo", "Repo geschrieben: Monet=$isMonetEnabled, Custom=$isCustom, Style=$style, ColorInt=$colorInt")
        ...
        context.dataStore.edit { preferences ->
            ...
            preferences[PreferencesKeys.CUSTOM_COLOR] = colorInt.toLong() // Int in Long umwandeln zum Speichern

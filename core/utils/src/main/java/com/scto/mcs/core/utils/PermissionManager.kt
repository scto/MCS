...
object PermissionManager {
    ...
    // Kompatibilität für alte Code-Aufrufe
    ...
    /**
     * ✅ Intelligente Prüfung: Ob für den angegebenen Pfad Systemberechtigungen erforderlich sind
    ...
    fun isSystemPermissionRequiredForPath(context: Context, path: String): Boolean {
        // Stammverzeichnis des privaten Verzeichnisses abrufen .../Android/data/Paketname
        val appExternalDir = context.getExternalFilesDir(null)?.parentFile?.parentFile?.absolutePath

        // Wenn der Abruf fehlschlägt, aus Sicherheitsgründen standardmäßig Berechtigung anfordern
        if (appExternalDir == null) return true

        // Wenn der Pfad ein Unterverzeichnis des privaten Verzeichnisses ist, direkt ausnehmen
        if (path.startsWith(appExternalDir)) {
            return false
        }

        // Andere Verzeichnisse nach Version beurteilen

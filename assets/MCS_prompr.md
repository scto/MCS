...
* :core Submodule:
   * :core:common - Common Files
   * :core:data – Daten-Implementierung (Repositories, JGit, File-I/O)
   * :core:domain – Business Logik (Interfaces, Use Cases, Entities)
   * :core:editor - Zentrale Manager (Sora Editor Configs, Sora Editor LSP, Sora Editor Textmate)
   * :core:navigation - Navigation Setup & Router
   * :core:resourcess - Zentrale Ressourcen (Strings.xml, Color.xml, Theme.xml usw.)
   * :core:terminal - Terminal Code Files
...
Ziel: Implementierung der "Gehirn"-Komponenten als Singletons in den jeweiligen Core-Submodulen.
...
1. Kontext bewahren: Nutze /add [Dateipfad], bevor du eine neue Phase startest, damit Aider die Schnittstellen und die Modulstruktur kennt.
